  ...
  async responsibleTicket(req, res) {
    try {
      ...
      const responsibleCheck =
        await this.responsibleModel.getActiveResponsibleByTicket(req.params.id);

        const act_user = await this.userController.checkUserCreated(
            req.body.act_user,
      ...
      const removeResponsible = await responsibleCheck.filter(
        (x) => !req.body.responsible.find((y) => y.id_user === x.id_user)
      );
      ...
      return res
        .status(500)
        .send({ error: "Ocorreu um erro ao vincular o responsável do ticket" });
    }
  }
}
