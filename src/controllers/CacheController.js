  ...
  async cachePhase() {
    ...
    for await (const phase of phases) {
      ...
      for await (const ticket of tickets) {
        Object.keys(phase.sla).length > 0 &&
          (ticket.sla = this.slaController.ticketSLA(phase.id, ticket.id));
        ticket.responsibles =
      ...
      await redis.set(
        `msTicket:closeTickets:${phase.id}`,
        JSON.stringify(closed_tickets)
      );
      
      await redis.set(
        `msTicket:openTickets:${phase.id}`,
        JSON.stringify(open_tickets)
      );
      
      await redis.set(
        `msTicket:inProgressTickets:${phase.id}`,
        JSON.stringify(in_progress_tickets)
      );
